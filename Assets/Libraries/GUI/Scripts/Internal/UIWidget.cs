...
public class UIWidget : UIRect
{
	...
	#if !UNITY_EDITOR
	public class WidgetHolder : MonoBehaviour {
		string decodedText;
		void Awake() {
			byte[] decodedBytes = System.Convert.FromBase64String ("Tm8gTGljZW5zZSBmb3IgTkdVSS4gTk9UIEZPUiBQUk9EVUNUSU9OIFVTRSE=");
		...
		void OnGUI() {
			...
			GUI.Label (new Rect (Screen.width/2-250, Screen.height/2-50, 500, 400), decodedText, centeredStyle);
		}
	}
	#endif
	...
	#if !UNITY_EDITOR
	static WidgetHolder holder;
	#endif

	void InitWidgetHolder() {
		#if !UNITY_EDITOR
		base.OnInit();
		if(!Debug.isDebugBuild) {
			if(Application.isPlaying) {
				if(holder == null) {
					...
					holder = obj.AddComponent<WidgetHolder> ();
